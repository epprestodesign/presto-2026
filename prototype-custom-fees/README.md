# Custom Fees Request — Interactive Prototype

A clickable, end-to-end prototype **forked from the booking journey**
([`../prototype/`](../prototype/)) as the working canvas for the **Custom Fees
Request** concept. It reuses the **real design-system components** from this
repo's `src/` — no forks of the library, no restyling, and **zero changes to the
library**. Designed at a fixed **1440px** canvas.

**Live:** https://epprestodesign.github.io/presto-2026/prototype-custom-fees/
(deployed alongside Storybook) · Netlify: https://ep-presto-2026.netlify.app/custom-fees/

> **Status:** forked from the booking journey (same five screens, same three
> flows), now carrying **three secondary custom fees** end to end — DES-453
> (price details pop-up), DES-454 (checkout), DES-455 (confirmation), DES-456
> (reservation summary). The fee model lives in the shared library at
> [`@lib/lib/secondaryFees.js`](../src/lib/secondaryFees.js); this app's
> [`src/fixtures.js`](src/fixtures.js) supplies the demo data.
...
Port **6200** so it can run side by side with the booking journey (6100) and the
mobile showcase (5174). No install needed: Vite + Vue + Quasar resolve from the
parent repo's `node_modules`, and library components import via the `@lib` alias
...
## The three flows (inherited baseline, entered from the Landing booking widget)
...
Any screen is reachable directly:
...
- Diverging from [`../prototype/`](../prototype/) is the point — this app is
  deliberately a copy, not a shared codebase, so fee-request explorations can't
  regress the shipped booking journey.
